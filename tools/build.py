...
arrives. The .css sources are not copied; skin assets (images) are, except .svg ones, which
are inlined as data: URIs so an ornament costs no request either.
...
def inline_skin(path: Path) -> str:
    ...
    # a bare relative .svg file name (e.g. url('leaf.svg')) is inlined as a data: URI
    def inline_svg(m):
        ref = path.parent / m.group(2)
        if not ref.is_file():
            sys.exit(f'{path.name}: references {m.group(2)}, which does not exist')
        svg = ref.read_text(encoding='utf-8').strip().replace('"', "'")
        svg = re.sub(r'\s+', ' ', svg)
        for ch, enc in (('<', '%3C'), ('>', '%3E'), ('#', '%23')):
            svg = svg.replace(ch, enc)
        return f'url("data:image/svg+xml,{svg}")'
    css = re.sub(r"url\((['\"]?)([\w.-]+\.svg)\1\)", inline_svg, css)
    ...
    css = re.sub(r"url\((['\"]?)([\w.-]+\.(?:webp|png|jpe?g|gif|avif))\1\)", r"url(\1skins/\2\1)", css)
...
def build(src: Path, out: Path) -> None:
    ...
    if out.exists():
        shutil.rmtree(out)
    shutil.copytree(src, out, ignore=shutil.ignore_patterns('*.css', '*.svg', '.DS_Store'))
